...
class Trainer():
    ...
    def train_epoch(self):
        ...
        for batch in dataloader:
            ...
            score_pos, score_hyp_pos = self.graph_classifier(pos_graph)
            score_neg, score_hyp_neg = self.graph_classifier(neg_graph)
            ...
            if self.params.loss == "bce" or self.params.loss == "fl":
            ...
            hpy_loss = neg_sampling_loss(score_hyp_pos,score_hyp_neg)
            print(f"loss:{loss}, hpy_loss:{hpy_loss}")
            loss += 0.1*hpy_loss
            loss.backward()
            ...
            with torch.no_grad():
                ...
                # hpy_scores += score_hyp_pos.squeeze(1).detach().cpu().tolist() + score_hyp_neg.squeeze(1).detach().cpu().tolist()
        ...
        # hpy_auc = metrics.roc_auc_score(all_labels, hpy_scores)
        # hpy_auc_pr = metrics.average_precision_score(all_labels, hpy_scores)
        # hpy_train_result = {'hpy_auc': hpy_auc, 'hyp_auc_pr': hpy_auc_pr}
        #
        # logging.info(f'Epoch {self.epoch} Hpy Training Performance:{hpy_train_result}')
