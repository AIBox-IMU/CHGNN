...
# from .hyperbolic_loss import similarity_score
# from .com_hpy import FFTAttH
...
class CycleGNN(nn.Module):
    def __init__(self, params, relation2id) -> None:
        ...
        # self.hpy = FFTAttH(self.params).to(device=self.params.device)
    ...
    def forward(self, g: dgl.DGLGraph):
        ...
        right = torch.cat([target_edge_feats[:, 0, :], target_query_feats[:, 0, :], target_head_feats, target_tail_feats], dim=-1).unsqueeze(1)
        left = torch.cat([target_edge_feats[:, 1, :], target_query_feats[:, 1, :], target_tail_feats, target_head_feats], dim=-1).unsqueeze(1)
        ...
        # rel_labels = g.edata["type"][g.edata["target_edge"]]
        # hyp_node = self.hpy.att_get_queries(nfeats[g.edges()[0][g.edata["target_edge"]]].clone().detach(),rel_labels)
        # hyp_node = hyp_node.reshape(g.batch_size, 2, -1)
        # hyp_score = similarity_score(hyp_node[:, 0],hyp_node[:, 1])
