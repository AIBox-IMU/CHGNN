...
def logmap_0(a):
    """projects the embedding 𝑎 back to Real Euclidean space"""

    # project an embedding from complex to real
    real_a = torch.fft.irfft(a, norm="ortho")
    # project an embedding from hyperbolic to euclidean
    norm_a = a.norm(dim=-1, p=2, keepdim=True).clamp_min(MIN_NORM)
    normalized_a = real_a / norm_a

    # realize artanh
    x = norm_a.clamp(-1 + 1e-5, 1 - 1e-5)
    dtype = x.dtype
    x = x.double()
    y = (torch.log_(1 + x).sub_(torch.log_(1 - x))).mul_(0.5).to(dtype)
    return y * normalized_a
...
def mobius_mul(W,a):
    """hyperbolic linear transformation, named Möbius matrix-vector multiplication,
        performs multiplication between a matrix W and an embedding a.
    """
    # Compute the dot product of W and a
    W_dot_a = torch.matmul(W, a)

    # Compute the norms
    norm_W_dot_a = torch.norm(W_dot_a)
    norm_a = torch.norm(a)

    # Compute the ratio of norms
    ratio = norm_W_dot_a / norm_a

    # Compute the hyperbolic tangent of the product of the ratio and inverse hyperbolic tangent
    scaling_factor = torch.tanh(ratio * torch.atanh(norm_a))

    # Normalize the vector W_dot_a
    normalized_W_dot_a = W_dot_a / norm_W_dot_a

    # Compute the final result
    result = scaling_factor * normalized_W_dot_a
