...
def main(params):
    ...
    isHit20List = [x for x in ranks if x <= 20]
    isHit30List = [x for x in ranks if x <= 30]
    ...
    hits_20 = len(isHit20List) / len(ranks)
    hits_30 = len(isHit30List) / len(ranks)

    mrr = np.mean(1 / np.array(ranks))
    logger.info(f'Testing time used: {end - start} s')
    logger.info(f'MRR | Hits@1 | Hits@3 | Hits@5 | Hits@10 | Hits@20 | Hits@30 : {mrr} | {hits_1} | {hits_3} |{hits_5} | {hits_10} | {hits_20} | {hits_30}')
